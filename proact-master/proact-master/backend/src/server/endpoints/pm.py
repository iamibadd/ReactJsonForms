...
from cache import dfm as dfm_cache_key, alignments, performance_metrics, aligned_times, ocel_performance_metrics, monitoring_results, action_engine_results, impact_analysis_results, build_aim_results
...
from worker.tasks.monitoring import evaluate_constraint_graphs_task
from worker.tasks.action import execute_action_engine_task
from worker.tasks.impact import execute_impact_analysis_task
...
@router.get('/monitoring', response_model=TaskStatus[Any])
def evaluate_constraint_graphs(ocel: str = Depends(ocel_filename_from_query), constraint_graphs: str = Query(example="[]"),
                                  task_manager: TaskManager = Depends(get_task_manager)):
    """
    Async: Tries to load the given OCEL from the data folder, calculates the DFM and the threshold associated with
    each edge, node and trace.
    ...
    :return: Taskstatus (potentially containing the previously computed (partial) result) of dfm construction task
    """
    constraint_monitoring_task = TaskDefinition(ocel,
                                       TaskName.CONSTRAINT_MONITORING.with_attributes(
                                           monitoring_ocel=ocel,
                                           constraint_graphs=constraint_graphs),
                                       evaluate_constraint_graphs_task, [ocel, constraint_graphs],
                                       monitoring_results(ocel, constraint_graphs),
                                       result_version="3")
    return task_manager.cached_task(constraint_monitoring_task)

@router.get('/action', response_model=TaskStatus[Any])
def execute_action_engine(cis: str = Query(example="[]"), constraint_patterns: str = Query(example="[]"), action_graph: str =Query(example="[]"), action_conflicts: str = Query(example="[]"), task_manager: TaskManager = Depends(get_task_manager)):
    """
    Async: Tries to load the given OCEL from the data folder, calculates the DFM and the threshold associated with
    each edge, node and trace.
    :param ocel: Path to ocel
    ...
    :return: Taskstatus (potentially containing the previously computed (partial) result) of dfm construction task
    """
    action_engine_task = TaskDefinition(cis,
                                       TaskName.ACTION_ENGINE.with_attributes(
                                           cis=cis,
                                           constraint_patterns=constraint_patterns),
                                       execute_action_engine_task, [cis, constraint_patterns, action_graph, action_conflicts],
                                       action_engine_results(cis, constraint_patterns, action_graph, action_conflicts),
                                       result_version="9")
    return task_manager.cached_task(action_engine_task)


@router.get('/impact', response_model=TaskStatus[Any])
def execute_impact_analysis(ocel: str = Depends(ocel_filename_from_query), ai: str = Query(example="[]"), change: str = Query(example="[]"), performance_analysis_window: str=Query(example="[]"), task_manager: TaskManager = Depends(get_task_manager)):
    """
    Async: Tries to load the given OCEL from the data folder, calculates the DFM and the threshold associated with
    each edge, node and trace.
    :param ocel: Path to ocel
    ...
    :return: Taskstatus (potentially containing the previously computed (partial) result) of dfm construction task
    """
    impact_analysis_task = TaskDefinition(ocel,
                                       TaskName.IMPACT_ANALYSIS.with_attributes(
                                           ocel=ocel, ai=ai,change=change, performance_analysis_window=performance_analysis_window),
                                       execute_impact_analysis_task, [ocel, ai, change, performance_analysis_window],
                                       impact_analysis_results(ocel, change, performance_analysis_window),
                                       result_version="7")
    return task_manager.cached_task(impact_analysis_task,True)
