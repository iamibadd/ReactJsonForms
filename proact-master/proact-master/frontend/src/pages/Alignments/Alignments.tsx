...
export const Alignments = connect<StateProps, DispatchProps, AlignmentProps, RootState>(mapStateToProps, mapDispatchToProps)((props: Props) => {
            ...
            alignmentData.forEach((traceWithAlignments: any) => {
                Object.keys(traceWithAlignments).forEach((objectType) => {
                    ...
                    if (alignment) {
                        ...
                        if (alignment['log_alignment'][0].activity === "|PROACT_START|")
                            alignment['log_alignment'].shift()
                        if (alignment['log_alignment'][alignment['log_alignment'].length - 1].activity === "|PROACT_END|")
                            alignment['log_alignment'].pop()
                        if (alignment['model_alignment'][0].activity === "|PROACT_START|")
                            alignment['model_alignment'].shift()
                        if (alignment['model_alignment'][alignment['model_alignment'].length - 1].activity === "|PROACT_END|")
    ...
    return (
        <div className="DefaultLayout-Container">
            <ProActNavbar lowerRowSlot={navbarItems} />
                    ...
                    if (alignment) {
                        if (alignment['log_alignment'].length === alignment['model_alignment'].length) {
                            ...
                            log_indices.forEach((index: number) => {
                                ...
                                if (index === 0) {
                                    console.log("something went wrong? ProAct start should always be in the log")
                                } else if (index === alignment['log_alignment'].length - 1) {
                                    console.log("something went wrong? ProAct end should always be in the log")
                                } else {
                            ...
                            // Since ProAct currently works with "interval" events and not with atomic ones, we only consider one
