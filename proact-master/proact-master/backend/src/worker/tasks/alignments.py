...
def rearrange_alignment(alignment: List[List[Tuple[str, str]]]) -> TraceAlignment:
    ...
    :return: Rearranged trace alignment type used by proact
    """
...
def build_petrinet(dfg):
    ...
    node_places = {}
    proact_start_place = None
    proact_end_place = None
    for (i, node) in enumerate(dfg.nodes):
        node_places[i] = add_place(net, name=node)
        if node == START_TOKEN:
            proact_start_place = node_places[i]
        elif node == STOP_TOKEN:
            proact_end_place = node_places[i]

    assert(proact_start_place is not None and proact_end_place is not None)

    # add artificial start to net because PROACT_START node will be target node of the PROACT_START transition
    petrinet_start_place = add_place(net)
    add_connected_transition(petrinet_start_place, proact_start_place, START_TOKEN, START_TOKEN, net)
