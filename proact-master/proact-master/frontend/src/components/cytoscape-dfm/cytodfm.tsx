...
export type DirectlyFollowsMultigraph = {
    ...
    traces: [
        {
            // Sometimes, we used actions synonymous to activity. If you keep on developing ProAct, you would probably want to refactor that :D
            actions: number[]
            thresholds: {
                [key: string]: {
                    count: number
...
export const SimpleCytoDFM = forwardRef((props: CytoDFMProps, ref: ForwardedRef<CytoDFMMethods | undefined>) => {
    ...
    const [elements, legendObjectTypeColors] = useMemo(() => {
        ...
        const elements: cytoscape.ElementDefinition[] = filteredNodes.concat([]).concat(links).concat([]);
    ...
    function onNodeTap(event: EventObject) {
        const selectedNodeNumberId = event.target.data().numberId;
        setSelection({
            selectedNode: selectedNodeNumberId,
            selectedEdge: null
        });
        if (props.onNodeSelect) {
            const nodeLabel = props.dfm?.nodes[selectedNodeNumberId].label;
            if (nodeLabel) {
                props.onNodeSelect(nodeLabel);
            }
        }
    }
