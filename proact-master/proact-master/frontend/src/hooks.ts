...
export function useIntervalTriggered(callback: () => any, trigger: boolean) {
    const savedCallback = useRef<() => any>();
    useEffect(() => {
        savedCallback.current = callback
    }, [callback]);
...
export function useAsyncAPITriggered<DataType>(
    endpoint: string,
    ...
    const state: AsyncApiState<DataType> = stateBackend
        ? stateBackend.state
        : componentState[0];
    const setState: PossibleAsyncFunction<AsyncApiState<DataType>, void> | null =
        stateBackend ? stateBackend.setState : componentState[1];
    ...
    const encoded_parameters = !parameters_empty
        ? Object.keys(parameters)
            .map((key) => `${key}=${encodeURIComponent(parameters[key])}`)
            .reduce((a, b) => a + "&" + b)
        : "";

    const uri = !parameters_empty
        ? API_BASE_URL + endpoint + "?" + encoded_parameters
    ...
    const tick = async () => {
        ...
        // else if (data.status === "running") {
        //     if (
        //         data.preliminary !== null &&
        //         !deepEqual(data.preliminary, state.preliminary)
        //     ) {
        //         setState({
        //             preliminary: data.preliminary,
        //             result: null,
        ...
        // } 
            ...
            console.log(
                "Task failed due to server related reasons. (Received 200 { status: 'failed' })"
            );
            setState({
                preliminary: null,
                result: null,
    ...
    useIntervalTriggered(tick, trigger);
